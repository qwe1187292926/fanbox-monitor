...
- **SQLite 永続化**：処理済み投稿、ダウンロード済みファイル、スキャン checkpoint、実行ログを記録
...
- **SQLite 永続化**：4テーブルスキーマ（`seen_posts`、`downloaded`、`cursor`、`run_log`）で重複排除状態、ダウンロード結果、最新スキャン checkpoint、実行ログを記録
